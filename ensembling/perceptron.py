	...
	def train( self, trainingData, trainingLabels, sample_weights=None):
		...
		for iteration in range(self.max_iterations):
			for i in range(len(trainingData)): 
				outputs = self.classify([trainingData[i]])
				initial_prediction = outputs[0]
				

				
				if initial_prediction != trainingLabels[i]:
					for key in self.features:
						self.weights[key] += trainingLabels[i] * trainingData[i][key]
					
				

	def sample_data(self, trainingData, trainingLabels, sample_weights):
		sampled_data_by_weight = []
		sampled_labels_by_weight = []
		all_index = []
		for i in range(len(trainingData)):
			all_index.append(i)

		choosen_indexes = np.random.choice(all_index, size = int(0.8 * len(trainingData)), replace = True, p = sample_weights)

		for choosen_index in choosen_indexes:
			sampled_data_by_weight.append(trainingData[choosen_index])
			sampled_labels_by_weight.append(trainingLabels[choosen_index])

		return sampled_data_by_weight, sampled_labels_by_weight
